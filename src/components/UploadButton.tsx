...
const UploadDropzone: FC = () => {
	...
	const onUpload = async (acceptedFiles: File[]) => {
		...
		setIsUploading(true)
		try {
			const res =  await axios.post<DBFileType>('/api/file/upload', formData, {
				onUploadProgress: (ev) => {
					setUploadProgress((ev.loaded / (ev.total ?? 10000)) * 100);
				}
			});
			setUploadProgress(100);

			router.replace(`/dashboard/${res.data.id}`);
		} catch (error) {
  ...
  return (
    ...
    >
      <DialogTrigger onClick={() => setIsOpen(true)} asChild>
        <Button>
          Upload PDF <Upload className='w-5 h-5 ml-2' />
        </Button>
      </DialogTrigger>
      <DialogContent>
        <UploadDropzone />
